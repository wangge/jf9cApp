...
$(function() {
    ...
    $.getJSON(MapiUrl + '/index.php?w=member_order_vr&t=order_info',{key:key,order_id:order_id}, function(result) {
    	if (result.datas.error) {
    		return ;
    	} 
    	result.datas.order_info.MUrl = MUrl;
        ...
        $.getJSON(MapiUrl + '/index.php?w=goods&t=store_o2o_addr', {store_id:result.datas.order_info.store_id},function(result){
        	if (result.datas.error) {
        		return ;
        	}
    ...
    function cancelOrderId(order_id) {
        $.ajax({
            type:"post",
            url:MapiUrl+"/index.php?w=member_order_vr&t=order_cancel",
    ...
    function toSend(){
    	...
    	$.ajax({
            type:"post",
            url:MapiUrl+"/index.php?w=member_order_vr&t=resend",
    ...
    function evaluationOrder() {
        ...
        location.href = 'member_vr_evaluation.html?order_id=' + orderId;
        
    }
    // 全部退款
    function allRefundOrder() {
        var orderId = $(this).attr('order_id');
        location.href = 'refund_all.html?order_id=' + orderId;
    }
