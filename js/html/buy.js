...
$(function() {
    ...
    $('#list-address-valve').click(function(){
        $.ajax({
            type:'post',
            url:MapiUrl+"/index.php?w=member_address&t=address_list",
            data:{key:key},
            dataType:'json',
            async:false,
        ...
        $.ajax({//提交订单信息
            ...
            success:function(result){
                ...
                if (result.datas.error) {
					...
					WTCloseWin();
          					/*setTimeout(function () {
          						 location.href = '../member/order_list.html?data-state=state_new';
          					},2000);*/
                    return false;
                }
                // 商品数据
                ...
                $('#container-fcode').find('.submit').click(function(){
                    ...
                    $.ajax({//提交订单信息
                        type:'post',
                        url:MapiUrl+'/index.php?w=member_buy&t=check_fcode',
                        dataType:'json',
    ...
    $('#payment-offline').click(function(){
        pay_name = 'offline';
        $('#select-payment-wrapper').find('.header-l > a').click();
        $('#select-payment-valve').find('.current-con').html('线下付款');
        $(this).addClass('sel').siblings().removeClass('sel');
    })
    ...
    $('#add_address_form').find('.btn').click(function(){
        if($.sValid()){
            ...
            $.ajax({
                type:'post',
                url:MapiUrl+"/index.php?w=member_address&t=address_add",
                data:param,
                dataType:'json',
                success:function(result){
                    if (!result.datas.error) {
    ...
    $('#invoice-need').click(function(){
        ...
        $.ajax({//获取发票内容
            type:'post',
            url:MapiUrl+'/index.php?w=member_invoice&t=invoice_content_list',
            data:{key:key},
            dataType:'json',
            success:function(result){
                checkLogin(result.login);
        ...
        $.ajax({
            type:'post',
            url:MapiUrl+'/index.php?w=member_invoice&t=invoice_list',
            data:{key:key},
            dataType:'json',
            success:function(result){
                checkLogin(result.login);
                ...
                var invoice_scroll = new IScroll('#invoice-list-scroll', { 
                ...
                $('.del-invoice').click(function(){
                    ...
                    $.ajax({
                        type:'post',
                        url:MapiUrl+'/index.php?w=member_invoice&t=invoice_del',
    ...
    $('input[name="inv_title_select"]').click(function(){
        if ($(this).val() == 'person') {
			$('#inv_title').attr('placeholder','输入姓名');
			$("input[name=inv_title]").val('');
			$('.inv-company').hide();
            $('#inv-title-li').show();
			$('.inv-title-li-person').show();
        } else {
			$('#inv_title').attr('placeholder','输入单位名称');
			$("input[name=inv_title]").val('');
            $('#inv-title-li').show();
			$('.inv-title-li-person').hide();
			$('.inv-company').show();
        }
    });
        ...
        if ($('#invoice-need').hasClass('sel')) {
            if (invoice_id == 0) {
                ...
                if (param.inv_title_select == 'person') {
                    param.inv_useridnum = $("input[name=inv_useridnum]").val();
					if (param.inv_title==''||param.inv_useridnum=='') {
						$.sDialog({
							skin:"red",
							content:'请输入姓名及身份证号码！',
							okBtn:true,
				...
				if (param.inv_title_select == 'company') {
                    param.inv_p_code = $("input[name=inv_p_code]").val();
                    param.inv_p_reg_addr = $("input[name=inv_p_reg_addr]").val();
                    param.inv_p_reg_phone = $("input[name=inv_p_reg_phone]").val();
                    param.inv_p_reg_bname = $("input[name=inv_p_reg_bname]").val();
                    param.inv_p_reg_baccount = $("input[name=inv_p_reg_baccount]").val();
					if (param.inv_title==''||param.inv_p_code=='') {
						$.sDialog({
							skin:"red",
							content:'请输入单位名称及纳税人识别号！',
							okBtn:true,
							cancelBtn:false
						});
						return false;
                ...
                $.ajax({
                    type:'post',
                    async:false,
                    url:MapiUrl+'/index.php?w=member_invoice&t=invoice_add',
                    data:param,
                    dataType:'json',
                    success:function(result){
    ...
    // 支付
    //var buy_step2 = 0;移到pay_common.js定义 0是关
   var is_tj_ok = false,tj_pay_sn="";
    $('#ToBuyStep2').click(function(){
      /*if(is_tj_ok&&tj_pay_sn!=''){
                   console.log(is_tj_ok);
                   console.log(tj_pay_sn);
         toPay(tj_pay_sn,'member_buy','pay');
        return false;
      }*/
        if (buy_step2) {
            $.sDialog({
        ...
        $.ajax({
            type:'post',
            url:MapiUrl+'/index.php?w=member_buy&t=buy_step2',
            data:{
                key:key,
                ifcart:ifcart,
                cart_id:cart_id,
