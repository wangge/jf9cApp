$(function() {
    var key = localStorage('seller_key');
	...
	$.ajax({
        type:'post',
        url:MapiUrl+"/index.php?w=seller_vr_bill&t=show_bill",
	...
	$('.btn').click(function(){
	    $.ajax({
			type:'post',
			url:MapiUrl+"/index.php?w=seller_vr_bill&t=confirm_bill",
			data:{key:key,ob_id:ob_id},
			dataType:'json',
			success:function(result){
					checkLogin(result.login);//检测是否登录了
					if (result.datas.error) {
                        ...
                        alert('操作成功！');
						location.href = 'store_bill_vr_info.html?ob_id='+ob_id;
					}
