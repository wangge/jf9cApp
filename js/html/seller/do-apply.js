    ...
    $.sValid.init({
        ...
        if($.sValid()){
            ...
            $.ajax({
                ...
                type: 'POST',
                dataType: 'json',
                data: $("form").serialize() + '&key=' + key + '&step=' + step + '&stype=0&client=' + Client,
                    ...
                    if (i < data.join_state.step) {
                        ...
                        var html = '<div class="alert-box"><i class="fa fa-check"></i>' + data.join_state.msg + '</div>';
                         if (data.join_state.do == 0) {
                            html +='<a id="reapply_btn" href="do-apply.html?do=reapply" class="btn">重新申请</a>';
                         }
                        $("fieldset.step-" + i).html(html);
                        $("fieldset.step-" + i).show();
                    }
        ...
        if(curr_step==2){
            $.sValid.init({
                ...
                messages:{
                    company_name: "请填写公司名",
                    area_info: "请选择公司所在地",
                    company_address_detail: "请填写公司详细地址",
                    //company_phone: "公司电话必填!",
                    contacts_name: "请填写联系人姓名",
                    contacts_phone:{
                        required:"请填写联系人手机号码",
                        mobile:"请正确输入手机号码"
                    },
                    ...
                    business_licence_number_elc : "请上传营业执照图片",
                    business_licence_number : "请填写营业执照号（副本）"
                },
                callback:function (eId,eMsg,eRules){
                    if(eId.length >0){
                        var errorHtml = "";
