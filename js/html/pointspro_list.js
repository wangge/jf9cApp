...
var key = $api.getStorage("key");//getQueryString('key');
...
function get_list() {
    ...
    $.getJSON(MapiUrl + '/index.php?w=pointprod&t=index' + window.location.search.replace('?','&'), param, function(result){
    	if(!result) {
    		result = [];
    		result.datas = [];
...
function search_show() {
    	...
    	$('#search_submit2').click(function(){
    		...
    		window.location.href =  'pointspro_list.html' + queryString;
    	});
	...
	if(o==2&&k=='isable')
	{
		...
		if(key== undefined||key=='')
		{
			 $.sDialog({
                        ...
                        cancelBtn: true,
                        okFn: function() {
							$api.setStorage('redirect_uri','/html/pointspro_list.html');
