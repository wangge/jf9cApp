...
if(cart_count > 0) {
	...
	if (gc_name != '') {
    	$(".product-title").html(gc_name);
    }else {
		if (keyword != '') {
			$(".product-title").html(keyword);
		}else{
			$(".product-title").html("商品");
		}
	}
    $.animationLeft({
        valve : '#search_show',
...
function get_list() {
    ...
    $.getJSON(MapiUrl + '/index.php?w=goods&t=goods_list' + window.location.search.replace('?','&'), param, function(result){
    	if(!result) {
    		result = [];
    		result.datas = [];
...
function search_show() {
    $.getJSON(MapiUrl + '/index.php?w=index&t=search_show', function(result) {
    	...
    	$('#search_submit').click(function(){
    		...
    		window.location.href =  'goods_list.html' + queryString;
    	});
