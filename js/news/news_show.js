...
$(function(){
	...
	if (article_id=='') {
		...
		$.ajax({
			url:MapiUrl+"/index.php?w=news_article&t=news_show",
			type:'get',
...
function loadPLlist() {
  ...
  param.type = 'article';
	...
	$.getJSON(MapiUrl+"/index.php?w=news_article&t=comment_list" + window.location.search.replace("?", "&"), param, function(e) {
	/*	if (!e) {
			e = [];
			e.datas = [];
	...
	var key = $api.getStorage('key');
	 if (!key) {
		...
		if (!conFlg) {
			conFlg = true;
			 $.ajax({
            url:MapiUrl+"/index.php?w=news_article&t=article_up",
            type:"post",
            dataType:"json",
...
function _itemz(id,his){
	...
	if ($api.getStorage(keys) == 1) {
		return false;
	} else {
					 $.ajax({
            url:MapiUrl+"/index.php?w=news_article&t=comment_up",
            type:"post",
            dataType:"json",
