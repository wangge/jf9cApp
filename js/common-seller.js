...
function checkLogin(state){
    ...
    api.openWin({
        name: 'LoginTo',
        url: 'widget://html/seller/login.html',
        animation: {
              type:"fade",
              subType:"from_right",
              duration:300
...
function getSearchName() {
	...
	if (keyword == '') {
	    if($api.getStorage('deft_key_value') == '') {
...
function checkSellerLogin(state) {
    if (state == 0) {
		...
		$api.rmStorage("seller_key");
		$api.rmStorage("is_seller");
        location.href = '../html/seller/login.html';
        return false;
    } else {
        return true;
    }
 ...
 */
 function getChatCount() {
     if ($('#header').find('.message').length > 0) {
         var key = $api.getStorage('key');
         if (key !== null) {
             $.getJSON(MapiUrl+'/index.php?w=seller_chat&t=get_user_list', {key:key}, function(result){
...
function play_audio(){//提示声音
    if ( chat_audio === 1 ) {
        $.ajax({
            url: ShopUrl+'/api/im/static/js/soundjs.min.js',
            dataType: "script",
            async: false
       });
        createjs.Sound.addEventListener("fileload", handleLoadComplete);
        createjs.Sound.registerSound({src:ShopUrl+"/api/im/static/msg.ogg", id:"sound"});
...
(function($) {
     ...
     */
        ...
        goodsClassSelected: function(options) {
            ...
            var ASDEEP = 1;
            var ASINIT = true;
            var skey = $api.getStorage("seller_key")
            function _classinit() {
                if ($("#areaSelected").length > 0) {
                    $("#areaSelected").remove()
                }
            ...
            function _getGoodsClassList() {
                $.ajax({
                    type: "post",
                    url: MapiUrl + "/index.php?w=seller_goods_class&t=class_list",
                    data: {
                        gc_id: ASID,
                        deep:ASDEEP,
                        client:'mobile',
